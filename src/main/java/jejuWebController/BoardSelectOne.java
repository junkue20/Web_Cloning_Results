...
public class BoardSelectOne extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// 이전, 다음글
		long prevNo =MyBatisContext.getSqlSession()
				.getMapper(BoardMapper.class).selectPrevBoardOne(no);
		long nextNo = MyBatisContext.getSqlSession()
				.getMapper(BoardMapper.class).selectNextBoardOne(no);
		// no값 전송
		request.setAttribute("prevNo", prevNo);
		request.setAttribute("nextNo", nextNo);
		
		// 3. no값을 이용해서 mapper호출 후 결과 받기
		Board board = MyBatisContext.getSqlSession()
				.getMapper(BoardMapper.class).selectBoardOne(no);
